...
//require_once 'DB.php';
require_once '/usr/share/adbanking/web/ad_compensation_siege/app/erreur.php';
...
function passageEcrituresComptablesAuto($type_oper, $montant, &$comptable_his, $array_cptes=NULL, $devise=NULL, $date_compta=NULL,$info_ecriture=NULL,$infos_sup=NULL) {
  ...
  if(empty($id_exo)) {
    ...
    $id_exo = $exo_encours[0]["id_exo_compta"];
  }
...
function getExercicesComptables($id_exo=NULL) {
  ...
  */
  ...
  if ($id_exo)
    ...
    $sql .= " AND (EXTRACT(YEAR FROM date_deb_exo) = EXTRACT(YEAR FROM date(now())) OR EXTRACT(YEAR FROM date_fin_exo) = EXTRACT(YEAR FROM date(now())))";
 ...
 * @since 1.0
 */
function phpseclib_Decrypt_CSA($ciphertext, $password="") {
  // Include SSH library
  /*
  ...
  */
  ...
  return Decrypte_CSA($ciphertext, $password);
}

/**
 * Permet de decrypter un texte
 */
function Decrypte_CSA($Texte,$Cle) {
  $Texte = GenerationCle_CSA(base64_decode($Texte),$Cle);
  $VariableTemp = "";
 ...
 */
function GenerationCle_CSA($Texte,$CleDEncryptage) {
