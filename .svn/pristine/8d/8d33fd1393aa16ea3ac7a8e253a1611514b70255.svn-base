...
v_montant_solde =0;
    --IF (in_id_saison = 0) THEN
    select into v_id_benef_verif ,v_qty,v_qty_paye, v_montant_paye1, v_montant_avance1, v_montant_solde1 id_benef, qty,sum(qty_paye) as qty_paye, (montant_avance+sum(montant_solde)) as montant_total, montant_avance, sum(montant_solde) as montant_solde from commande where id_produit = ligne1.id_produit and id_benef = id_ben_temp GROUP BY id_benef,qty,montant_avance; -- and id_saison IN (SELECT id_saison from ec_saison_culturale where id_annee = in_id_annee);
    /*END IF;
    IF (in_id_saison = 1) THEN
...
DROP TYPE IF EXISTS data_qtite_zone CASCADE;

CREATE TYPE data_qtite_zone AS (
      id_province int,	
      nom_province text,
      id_commune int,
      nom_commune text,
