...
CREATE OR REPLACE FUNCTION makeNumCompletCpte (bigint, bigint, bigint) RETURNS text AS '
  BEGIN
    ...
    ELSIF $1 = 2 THEN
      RETURN lpad((SELECT code_banque FROM ad_agc WHERE id_ag=NumAgc()), 2, ''0'')||lpad((SELECT code_ville FROM ad_agc WHERE id_ag=NumAgc()), 2, ''0'')||''-''||lpad($2, 5, ''0'')||lpad($3, 2, ''0'')||''-''||to_char((to_number(lpad((SELECT code_banque FROM ad_agc WHERE id_ag=NumAgc()), 2, ''0'')||lpad((SELECT code_ville FROM ad_agc WHERE id_ag=NumAgc()), 2, ''0'')||lpad($2, 5, ''0'')||lpad($3, 2, ''0''),''99999999999'')%97), ''FM00'');
    ELSIF $1 = 3 THEN
      RETURN lpad((SELECT code_banque FROM ad_agc WHERE id_ag=NumAgc()), 3, ''0'')||''-''||lpad($2, 7, ''0'')||''-''||lpad($3, 2, ''0'');
    ELSIF $1 = 4 THEN
      RETURN lpad((SELECT code_antenne FROM ad_agc WHERE id_ag=NumAgc()),NumAgc(),''0'')||''-''||lpad($2, 6, ''0'')||''-''||lpad($3, 2, ''0'')||''-''||to_char((to_number(lpad(NumAgc(),3,''0'')||lpad($2, 6, ''0'')||lpad($3, 2, ''0''),''99999999999'')%97), ''FM00'');
    END IF;
