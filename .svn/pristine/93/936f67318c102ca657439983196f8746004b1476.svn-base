...
/**
 * [110] Approbation dossier de crédit
 * Cette opération comprends les écrans :
 * - Apd-1 : sélection d'un dossier de crédit
 * - Apd-2 : approbation d'un dossier de crédit
 * - Apd-3 : affichage de l'échéancier
 * - Apd-4 : blocage des garanties numéraires
 * - Apd-5 : confirmation approbation d'un dossier de crédit
 * - Apd-6 : affichage des garanties
 * - Apd-7 : ajout de garanties
 * - Apd-8 : modification de garanties
 * - Apd-9 : suppression de garanties
 * - Apd-10 : confirmation ajout, modification ou suppression de garanties
 * @package Credit
 */
...
if ($global_nom_ecran == "Apd-1") {
  ...
  // Récupération des dossiers individuels dans ad_dcr en attente de décision ou en attente de Rééch/Moratoire et periodicite pas 'En une fois'
  $whereCl=" AND is_ligne_credit='f' AND mode_calc_int!=5 AND ((etat=1) OR (etat=7 and periodicite <> 6))";
  ...
  if ($SESSION_VARS['infos_client']['statut_juridique'] == 4) {
    ...
    foreach($dossiers_fictifs as $id=>$value) {
      ...
      if (($val['is_ligne_credit'] != 't') AND ($val['id_dcr_grp_sol'] == $id) AND ($val['etat'] == 1 OR ($val['etat'] == 7 && $val['periodicite'] != 6) )) {
  ...
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Apd-2");
...
/*{{{ Apd-2 : Approbation d'un dossier de crédit */
else if ($global_nom_ecran == "Apd-2") {
  global $adsys;

  // Si on vient de Apd-1, on récupère les infos de la BD
  if (strstr($global_nom_ecran_prec,"Apd-1")) {
    ...
    elseif($SESSION_VARS['dossiers'][$HTML_GEN_LSB_id_doss]['gs_cat'] == 2 ) { // GS avec dossiers multiples
      ...
      $dossiers_membre = getDossiersMultiplesGS($global_id_client);
      foreach($dossiers_membre as $id_doss=>$val) {
        if ($val['id_dcr_grp_sol']==$SESSION_VARS['dossiers'][$HTML_GEN_LSB_id_doss]['id'] and ($val['etat']==1 or ($val['etat']==7 && $val['periodicite'] != 6) )) {
    ...
    foreach($SESSION_VARS['infos_doss'] as $id_doss=>$infos_doss) {
     ...
     //si l'etat du credit est en attente de rééchel/Moratoire ne pas prendre en compte cette partie. #531
       if($etat_dossier != 7)
       {
  ...
  //on revient de Apd-6 aprés consultation des garanties
  if (strstr($global_nom_ecran_prec,"Apd-6")) {
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss)
  {
    ...
    // commissions:
    $comm_init = getfraisTarification('CRED_COMMISSION', $prc_commission, $val_doss["mnt_dem"], $mnt_commission);
    ...
    $Myform->setFieldProperties("mnt_commission".$id_doss,FIELDP_DEFAULT, $comm_init);

    // assurances
    $assurance_init = getfraisTarification('CRED_ASSURANCE', $prc_assurance, $val_doss["mnt_dem"], $mnt_assurance);
    ...
    //Verification du nombre de jour pour bloquer le credit avant echeance par rapport au maximum
    ...
    if ($SESSION_VARS['infos_doss'][$id_doss]['etat'] == 7) { // Rééchelonnement/ Moratoire
      ...
      // Recup la duree souhaité de reechlonnement
      $his_obj = Historisation::getListDossierHis($id_doss, Historisation::MOD_TYPE_REECH, 'f');
      ...
      $nbr_echeances_min = $duree['nbr_echeances_min'];


      // #433 : Le champ echeances_restant garde le nombre d’échéances restantes
      $Myform->addField("echeances_restant".$id_doss, _("Nombre d’échéances restantes"), TYPC_INT);
      $Myform->setFieldProperties("echeances_restant".$id_doss,FIELDP_IS_REQUIRED,true);
      ...
      $Myform->setFieldProperties("nbr_echeances_souhaite".$id_doss,FIELDP_DEFAULT, $tmp_reech_duree);
    ...
    if ($SESSION_VARS['infos_doss'][$id_doss]['gs_cat'] == 1) {
      ...
      foreach($SESSION_VARS['infos_doss'][$id_doss]['doss_fic'] as $id_fic=>$val_fic) {
        ...
        if (isDcrDetailObjCreditLsb()) {
          ...
          $Myform->setFieldProperties("obj_dem_fic" . $id_fic, FIELDP_JS_EVENT, array("onchange" => "setDetailObj2$id_fic();"));
          $Myform->addField("detail_obj_dem_2_fic" . $id_fic, _("Détail demande 2"), TYPC_LSB);
          $Myform->setFieldProperties("detail_obj_dem_2_fic" . $id_fic, FIELDP_DEFAULT, $val_fic['detail_obj_dem_2']);
        }
      ...
      $js_3 .= "function setDetailObj2$id_fic(){ \n var myArray2 = [\n";
      ...
      $js_3 .= " if( lookup2(document.ADForm.HTML_GEN_LSB_obj_dem_fic$id_fic.value, myArray2 ) != false ) { \n";
      $js_3 .= "     var select2 = document.ADForm.HTML_GEN_LSB_detail_obj_dem_2_fic$id_fic;
                      select2.options.length=0;";
      $js_3 .= "    var det_cred2=(lookup(document.ADForm.HTML_GEN_LSB_obj_dem_fic$id_fic.value, myArray2 ));\n
                     document.ADForm.HTML_GEN_LSB_detail_obj_dem_2_fic$id_fic.options[document.ADForm.HTML_GEN_LSB_detail_obj_dem_2_fic$id_fic.length] = new Option(\"[Aucun]\" ,0, false, false);
                     for(var i in det_cred2)
                     {
                        document.ADForm.HTML_GEN_LSB_detail_obj_dem_2_fic$id_fic.options[document.ADForm.HTML_GEN_LSB_detail_obj_dem_2_fic$id_fic.length] = new Option(det_cred2[i] ,i, false, false);
      ...
      $js_onload .= "\n setDetailObj2$id_fic();document.ADForm.HTML_GEN_LSB_detail_obj_dem_2_fic$id_fic.value = '".$val_fic['detail_obj_dem_2']."';";
      $Myform->addJS(JSP_FORM, "det_obj_gs2", $js_3);
      // fin ticket At-97
    ...
    // Init
    if(is_null($tmp_reech_duree)) $tmp_reech_duree = 0;

    $js_check .="\t\t
      if (document.ADForm.nbr_echeances_souhaite$id_doss) {
        ...
        if(selected_echeances < 1 || selected_echeances < min_echeances || selected_echeances > max_echeances) {\n
            msg +='- ".$msgControlDuree."\\n';
            document.ADForm.nbr_echeances_souhaite$id_doss.value = $tmp_reech_duree;\n
            ADFormValid=false;\n
        }
      }
      ";
  ...
  if ($can_mob_gar) {
    ...
    $Myform->setFormButtonProperties("mobiliser_gar", BUTP_PROCHAIN_ECRAN, "Apd-6");
  ...
  // Propriétés des boutons
  $Myform->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Gen-11");
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Apd-3");
  ...
  $js_mnt_init .="\t\t\t var mnt_octr_saisie = parseFloat(recupMontant(eval('document.ADForm.'+cre_mnt_octr).value)); \n";

  // assurances
  $js_mnt_init .="\t\t\tvar mnt_assurance_pour = ".$prc_assurance." * mnt_octr_saisie; \n";
  ...
  // commissions
  $js_mnt_init .="\t\t\tvar mnt_comm_pour = " .$prc_commission. " * mnt_octr_saisie; \n";
...
/*{{{ Apd-3 : Affichage de l'échéancier */
else if ($global_nom_ecran == "Apd-3") {
  $HTML_code = '';
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    if ($val_doss["last_etat"] == 1) { // L'état du dossier est En attente de décision
      ...
      if ($SESSION_VARS['infos_doss'][$id_doss]["gs_cat"] == 1) {
        foreach($SESSION_VARS['infos_doss'][$id_doss]['doss_fic'] as $id_fic=>$val_fic) {
          //$SESSION_VARS['infos_doss'][$id_doss]['doss_fic'][$id_fic]['id_bailleur_fic'] = $ {'id_bailleur_fic'.$id_fic};
      ...
      $parametre ["titre"] = _ ( "Nouvel échéancier réel de remboursement du dossier N° " ) .$id_doss;
      ...
      $parametre["perio"] = $SESSION_VARS['infos_prod']["periodicite"];
    ...
    elseif($val_doss["last_etat"] == 7) { // 7: En attente de Rééch/Moratoire
      $differe_jours = 0;
      $differe_ech = 0;

      $nbr_echeances_souhaite = $ {'nbr_echeances_souhaite'.$id_doss}; //Duréé nouvelle de crédit

      if(is_null($nbr_echeances_souhaite)) $nbr_echeances_souhaite = 0;
      ...
      // Echéancier
      $echeancier = calcul_echeancier_theorique_raccourci($SESSION_VARS['infos_doss'][$id_doss]['id_prod'], $SESSION_VARS['infos_doss'][$id_doss]["nouveau_cap"], $SESSION_VARS['infos_doss'][$id_doss]['nbr_echeances_souhaite'], $differe_jours, $differe_ech, NULL, 1, $id_doss);
      ...
      if (!$SESSION_VARS['infos_doss'][$id_doss]['id_ech']) // Récupération dernière échéance remboursée
    ...
    $HTML_code .= HTML_echeancier_raccourci($parametre,$echeancier,$id_doss);
  }
  ...
  //$formEcheancier->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Apd-4");
...
/*{{{ Apd-4 : Blocage des garanties numéraires */
else if ($global_nom_ecran == "Apd-4") {
  ...
  // Propriétés des boutons
  $formConf->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Apd-5");
...
/*{{{ Apd-5 : Confirmation approbation d'un dossier de crédit */
else if ($global_nom_ecran == "Apd-5") {
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    ...
    elseif($val_doss['last_etat'] == 7) { // dossier en attente de rééchelonnement
      $DATA[$id_doss]['last_etat'] = $val_doss['last_etat'];
    ...
    } // Fin si en attente de rééchelonnement
  } // Fin parcours dossiers
...
/*{{{ Apd-6 : Affichage des garanties */
else if ($global_nom_ecran == "Apd-6") {
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    // Si on vient de Apd-2 alors mémoriser les données modifiables de Apd-2
    if (strstr($global_nom_ecran_prec,"Apd-2")) {
    ...
    if ($val_doss['etat'] == 1)
      ...
      if ($value['type'] != '') { // Si la garantie n'est pas supprimée
        ...
        $xtHTML .= "<td><A href=\"$PHP_SELF?m_agc=".$_REQUEST['m_agc']."&prochain_ecran=Apd-8&benef=".$id_doss."&num_gar=".$key."\">"._("Mod")."</A></td>";
        $xtHTML .= "<td><A href=\"$PHP_SELF?m_agc=".$_REQUEST['m_agc']."&prochain_ecran=Apd-9&benef=".$id_doss."&num_gar=".$key."\">"._("Sup")."</A></td></tr>";
      }
    }
  }
  ...
  $Myform->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, 'Apd-2');
  $Myform->setFormButtonProperties("ajout_gar", BUTP_PROCHAIN_ECRAN, 'Apd-7');
...
/*{{{ Apd-7 : Ajout de garanties */
else if ($global_nom_ecran == "Apd-7") {
  ...
  $Myform->addFormButton(1,1,"valider", _("Valider"), TYPB_SUBMIT);
  $Myform->addFormButton(1,2,"retour", _("Retour"), TYPB_SUBMIT);
  $Myform->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, 'Apd-6');
  $Myform->setFormButtonProperties("retour", BUTP_CHECK_FORM, false);
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, 'Apd-10');
...
/*{{{ Apd-8 : Modification de garanties */
else if ($global_nom_ecran == "Apd-8") {
  $Myform = new HTML_GEN2();
...
/*{{{ Apd-9 : Suppression de garanties */
else if ($global_nom_ecran == "Apd-9") {
  $Myform = new HTML_GEN2();
  ...
  $Myform->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, 'Apd-6');
  $Myform->setFormButtonProperties("retour", BUTP_CHECK_FORM, false);
  $Myform->setFormButtonProperties("supprimer", BUTP_PROCHAIN_ECRAN, 'Apd-10');
...
/*{{{ Apd-10 : Confirmation ajout, modification ou suppression de garanties */
else if ($global_nom_ecran == "Apd-10") {
  ...
  $Myform->addButton(BUTTON_OK, 'Apd-6');
  $Myform->buildHTML();
