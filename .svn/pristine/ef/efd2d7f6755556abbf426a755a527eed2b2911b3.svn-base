...
/**
 * [126] Annulation de déboursement progressif d'un dossier de crédit
 * Cette opération comprends les écrans :
 * - Adp-1 : sélection d'un dossier de crédit
 * - Adp-2 : affichage du dossier de crédit
 * - Adp-3 : confirmation de l'annulation déboursement progressif
 * @package Credit
 */
...
if ($global_nom_ecran == "Adp-1") {
  ...
  $whereCl = "  AND (etat=13)"; // Le dossier doit être en déboursement progressif
  ...
  if ($SESSION_VARS['infos_client']['statut_juridique'] == 4) {
    ...
    foreach($dossiers_fictifs as $id=>$value) {
      ...
      if (($val['id_dcr_grp_sol'] == $id) AND ($val['etat'] == 13)) {
  ...
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Adp-2");
...
/*{{{ Adp-2 : Affichage du dossier */
else if ($global_nom_ecran == "Adp-2") {
  // Si on vient de Apd-1, on récupère les infos de la BD
  if (strstr($global_nom_ecran_prec,"Adp-1")) {
  ...
  // Création du formulaire
  $Myform = new HTML_GEN2(_("Annulation déboursement crédit"));
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    ...
    $Myform->addField("mnt_octr".$id_doss, _("Montant octroyé"), TYPC_MNT);
    $Myform->setFieldProperties("mnt_octr".$id_doss,FIELDP_IS_LABEL,true);
    $Myform->setFieldProperties("mnt_octr".$id_doss,FIELDP_DEFAULT, $val_doss['cre_mnt_octr'],true);

    $Myform->addField("cre_date_approb".$id_doss, _("Date approbation crédit"), TYPC_DTE);
    ...
    $Myform->addField("mnt_deb".$id_doss, _("Montant déboursé"), TYPC_MNT);
    $Myform->setFieldProperties("mnt_deb".$id_doss,FIELDP_IS_LABEL,true);
    $Myform->setFieldProperties("mnt_deb".$id_doss,FIELDP_DEFAULT, $val_doss['cre_mnt_deb'],true);
    ...
    $Myform->setFieldProperties("etat".$id_doss,FIELDP_DEFAULT,adb_gettext($adsys["adsys_etat_dossier_credit"][$val_doss['etat']]));
  	...
  	$include = $adsys["adsys_motif_etat_dossier_credit"];
      $tabMotif=array();
      foreach($include as $cle => $valeur) {
        $tabMotif[$cle]=adb_gettext($adsys["adsys_motif_etat_dossier_credit"][$cle]);
      }
...
// Test de la date d'annulation
  ...
  // Propriétés des boutons
  $Myform->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Gen-11");
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Adp-3");
  $Myform->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);
...
/*{{{ Adp-3 : Confirmation de l'annulation */
else if ($global_nom_ecran == "Adp-3") {
	$DATA = array();
	foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
		//Remplissage de $DATA avec les données postées au serveur.
		$DATA[$id_doss]["id_doss"] = $id_doss;
		$DATA[$id_doss]["etat"] = 5; // on met l'état à déboursé
	...
	$myErr = annulerDeboursementProgressif($DATA);
	if ($myErr->errCode != NO_ERR) {
		$html_err = new HTML_erreur(_("Echec lors de l'annulation de déboursement.")." ");
		$html_err->setMessage(_("Le déboursement n'a pas été annulé")." : ".$error[$myErr->errCode].$myErr->param);
		$html_err->addButton("BUTTON_OK", 'Gen-11');
		$html_err->buildHTML();
		echo $html_err->HTML_code;
	}else {
		$msg = new HTML_message(_("Confirmation annulation déboursement de crédit"));
		$msg->setMessage(_("L'annulation déboursement de crédit  est passé avec succès !"));
