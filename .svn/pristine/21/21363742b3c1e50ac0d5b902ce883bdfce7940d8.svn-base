...
/**
 * [139] Annulation d'un dossier de crédit
 * Cette opération comprends les écrans :
 * - Ruf-1 : sélection d'un dossier de crédit
 * - Ruf-2 : affichage du dossier de crédit
 * - Ruf-3 : confirmation de l'annulation
 * @package Credit
 */
...
if ($global_nom_ecran == "Ruf-1") {
    ...
    $whereCl = "  AND (etat=7) AND periodicite=6 "; // Le dossier doit être en attente de décision ou en attente de Rééch/Moratoire
    ...
    if ($SESSION_VARS['infos_client']['statut_juridique'] == 4) {
        ...
        foreach($dossiers_fictifs as $id=>$value) {
            ...
            foreach($dossiers_membre as $id_doss=>$val)
                if (($val['id_dcr_grp_sol'] == $id) AND (in_array($val['etat'], array(7))) AND $val['periodicite']==6 ) {
    ...
    $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Ruf-2");
...
/*{{{ Ruf-2 : Affichage du dossier */
else if ($global_nom_ecran == "Ruf-2")
{
    // Si on vient de Apd-1, on récupère les infos de la BD
    if (strstr($global_nom_ecran_prec,"Ruf-1")) {
    ...
    // Création du formulaire
    $Myform = new HTML_GEN2(_("Annulation rééchelonnement d'un dossier de crédit"));
    ...
    foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
        ...
        if (in_array($val_doss["etat"], array(7))) {
            $Myform->addField("nouv_duree_mois".$id_doss, _("Durée en mois souhaité"), TYPC_INT);
...
// Test de la date d'annulation
    ...
    // Propriétés des boutons
    $Myform->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Gen-11");
    $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Ruf-3");
    $Myform->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);
...
/*{{{ Ruf-3 : Confirmation de l'annulation */
else if ($global_nom_ecran == "Ruf-3")
{
    $moratoire=0;
