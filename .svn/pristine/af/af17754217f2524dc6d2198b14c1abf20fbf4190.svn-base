...
<xsl:template match="header_contextuel">
	...
	Type d'épargne ; <xsl:value-of select="translate(type_epargne,';','')"/>;
...
<xsl:template match="type_epargnes">
	<xsl:call-template name="titre1"><xsl:with-param name="titre" select="lib_type_ep"/></xsl:call-template>;
...
<xsl:template match="comptes">
	;<xsl:value-of select="translate(num_client,';','')"/>;<xsl:value-of select="translate(nom_client,';','')"/>;<xsl:value-of select="translate(num_compte,';','')"/>;<xsl:value-of select="translate(solde_compte,';','')"/>;
</xsl:template>
...
<xsl:template match="total">
	<xsl:call-template name="titre1"><xsl:with-param name="titre" select="'Total général;;'"/></xsl:call-template>;<xsl:value-of select="translate(tot_nb_compte,';','')"/>;<xsl:value-of select="translate(tot_solde_compte,';',';')"/>;
</xsl:template>
</xsl:stylesheet>
