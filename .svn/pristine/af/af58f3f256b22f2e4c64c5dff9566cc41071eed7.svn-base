...
/**
 * [608] Réalisation garanties
 *
 * Cette opération comprends les écrans :
 * - LRga-1 : sélection d'un dossier de crédit
 * - LRga-2 : sélection de la garantie
 * - LRga-3 : demande de confirmation réalisation de la garantie
 * - LRga-4 : confirmation de l'opération
 * @package Credit
 */
...
if ($global_nom_ecran == "LRga-1") {
  ...
  if($AG['realisation_garantie_sain'] == 't'){
  	...
  	$whereCl=" AND is_ligne_credit='t' AND mode_calc_int=5 AND ((etat=5) OR (etat=9)) AND cre_etat != $idEtatPerte";
  } else {
  	/* Recherche des dossiers individuels déboursés ou en attente de Rééch/Moratoire ni sain ni en perte */
        $whereCl=" AND is_ligne_credit='t' AND mode_calc_int=5 AND ((etat=5) OR (etat=9)) AND cre_etat > 1";
  }
  ...
  if ($SESSION_VARS['infos_client']['statut_juridique'] == 4) {
    ...
    foreach($dossiers_fictifs as $id=>$value) {
      ...
      if (($val['is_ligne_credit'] == 't') AND ($val['id_dcr_grp_sol'] == $id) AND ($val['etat'] == 5 or $val['etat'] == 9 ) AND $val_doss['cre_etat'] != $idEtatPerte) {
        // Liste des garanties du dossier
        $liste_gars = getListeGaranties($id_doss);
        ...
        foreach($liste_gars as $cle=>$valeur)
        if ( ($valeur['etat_gar'] == 3) OR  //garantie mobilisée 
 	         ($value['gar_num_encours']>0 AND $value['cpt_gar_encours'] == $valeur['gar_num_id_cpte_nantie']) ) {//garantie numéraire à constituer
  ...
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "LRga-2");
...
/*{{{ LRga-2 : Sélection de la garantie */
elseif($global_nom_ecran == "LRga-2") {
  // Si on vient de LRga-1, on récupère les infos de la BD
  if (strstr($global_nom_ecran_prec,"LRga-1")) {
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    ...
    $Myform->addHTMLExtraCode("cred","<table align=\"center\" valign=\"middle\" bgcolor=\"".$colb_tableau."\"><tr><td><b>".sprintf(_("Informations sur le Crédit du dossier N° %s"),$id_doss)."</b></td></tr></table>\n");
    ...
    foreach($liste_gars as $key=>$value) {
      /* Seules les garanties dont l'état est à 'Mobilisé' peuvent être réalisées */
        ...
        if (( $value['type_gar'] == 1 and $value['etat_gar'] == 3 AND  $value['gar_num_id_cpte_nantie'] != '' and $value['gar_num_id_cpte_prelev'] !='')
          ...
          $xtHTML .= "<td><A href=\"$PHP_SELF?m_agc=".$_REQUEST['m_agc']."&prochain_ecran=LRga-3&num_gar=".$key."&type_gar=".$value['type_gar']."&mnt_gar=".$value['montant_vente']."&is_gar_constituee=true\">"._("Réaliser")."</A></td>";
          $xtHTML .= "</TR> ";
        }
        elseif( $value['type_gar'] == 2 and  $value['etat_gar'] == 3 AND $value['gar_mat_id_bien'] != '') {
          ...
          $xtHTML .= "<td>".$value['devise_vente']."</td>";
          $xtHTML .= "<td>".adb_gettext($adsys["adsys_etat_gar"][$value['etat_gar']])."</td>";
          $xtHTML .= "<td><A href=\"$PHP_SELF?m_agc=".$_REQUEST['m_agc']."&prochain_ecran=LRga-3&num_gar=".$key."&type_gar=".$value['type_gar']."&mnt_gar=".$value['montant_vente']."\">Réaliser</A></td>";
          $xtHTML .= "</tr>";
        }
        elseif( $SESSION_VARS['infos_doss'][$id_doss]['gar_num_encours']>0 AND $SESSION_VARS['infos_doss'][$id_doss]['cpt_gar_encours'] == $value['gar_num_id_cpte_nantie']){
          ...
          if ($mnt_vente > 0 ) {
            ...
            $origine = $CPT_PRELEV_GAR["num_complet_cpte"]." ".$CPT_PRELEV_GAR['intitule_compte'] ;
	          ...
	          $xtHTML .= "<td>".$value['devise_vente']."</td>";
	          $xtHTML .= "<td>".adb_gettext($adsys["adsys_etat_gar"][$value['etat_gar']])."</td>";
	          $xtHTML .= "<td><A href=\"$PHP_SELF?m_agc=".$_REQUEST['m_agc']."&prochain_ecran=LRga-3&num_gar=".$key."&type_gar=".$value['type_gar']."&mnt_gar=".$mnt_vente."&is_gar_constituee=true\">"._("Réaliser")."</A></td>";
	          $xtHTML .= "</TR> ";
          }
        }
  ...
  /* Propriétés des boutons */
  $Myform->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, "Lcr-1");
...
/*{{{ LRga-3 : Demande de confirmation réalisation de la garantie */
elseif($global_nom_ecran == "LRga-3") {
  ...
  /* Propriétés des boutons */
  $Myform->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "LRga-2");
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "LRga-4");
  $Myform->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);
...
/*{{{ LRga-4 : Confirmation de l'opération */
elseif($global_nom_ecran == "LRga-4") {
  /* Devise courante : devise du produit de crédit associé */
  setMonnaieCourante($SESSION_VARS['infos_prod']["devise"]);

  /* Réalisation de la gaarantie */
  if ($SESSION_VARS['type_gar'] == 1)
    $myErr = realiseGarantie($SESSION_VARS['id_gar'], NULL, 608);
  else if ($SESSION_VARS['type_gar'] == 2)
    $myErr = realiseGarantie($SESSION_VARS['id_gar'], recupMontant($montant_net), 608);
  ...
  if ($myErr->errCode == NO_ERR) { /* La garantie a été réalisée avec succès */
    ...
    $html_err->addButton("BUTTON_OK", 'LRga-2');
    $html_err->buildHTML();
