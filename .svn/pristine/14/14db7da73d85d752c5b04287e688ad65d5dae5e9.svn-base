...
/**
* [133] Traitement pour remboursement anticipé
* Cette opération comprends les écrans :
* - Tre-1 : Sélection d'un dossier de crédit
* - Tre-2 : Affichage de l'échéancier de remboursement modifiable
* - Tre-3 : Affichage de l'échéancier à confirmer
* - Tre-4 : Confirmation de l'enregistrement du nouvel échéancier
* @package Credit
*/
...
if ($global_nom_ecran == "Tre-1") {
  ...
  if ($SESSION_VARS['infos_client']['statut_juridique'] == 4) {
    ...
    foreach($dossiers_fictifs as $id=>$value) {
      ...
      foreach($dossiers_membre as $id_doss=>$val)
        if (($val['id_dcr_grp_sol'] == $id) AND ($val['etat'] == 5 OR $val['etat'] == 7  OR $val['etat'] == 13)) {
  ...
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Tre-2");
...
/*{{{ Tre-2 : Affichage de l'échéancier de remboursement modifiable */
else if ($global_nom_ecran == 'Tre-2') {


  if (strstr($global_nom_ecran_prec,"Tre-1")) {
  ...
  if ($agc_data["tx_remb_anticipe"] == 't') {
    if (isset($SESSION_VARS['id_doss_anti'])){
      $id_doss = $SESSION_VARS['id_doss_anti'];
    }

    // Création tableau de 4 colonnes
    $table =& $formEcheancier->addHTMLTable("Int_anticipation", 5, TABLE_STYLE_ALTERN);
    $table->set_property("title","Calcul intérêt par anticipation");
    //$table->set_property("align", "center");
    //$table->set_property("border", $tableau_border);
    //$table->set_property("bgcolor", '#e0e0ff');

    $table->add_cell(new TABLE_cell(_("Dossier"), 1, 1));
    $table->add_cell(new TABLE_cell(_("Encours"), 1, 1));
    $table->add_cell(new TABLE_cell(_("Taux appliqué %"), 1, 1));
    $table->add_cell(new TABLE_cell(_("Intérêt pour anticipation"), 1, 1));
    $table->add_cell(new TABLE_cell(_("Intérêt pour anticipation actuel"), 1, 1));
    $mntCapEncours = getSoldeCapital($id_doss);

    if (isset($SESSION_VARS["tx_anticipation"])) {
      $tx_anticipation = $SESSION_VARS["tx_anticipation"];
      $mnt_anticipation = $mntCapEncours * ($tx_anticipation/100);
    }
    $info_doss = getDossierCrdtInfo($id_doss);

    $table->add_cell(new TABLE_cell($id_doss));
    $table->add_cell(new TABLE_cell_input_text("mnt_solde", 12, afficheMontant($mntCapEncours), "", "", true));
    $table->add_cell(new TABLE_cell_input_text("tx_anticipation", 12, $tx_anticipation, "MiseAJourMntAnticipation()"));
    $table->add_cell(new TABLE_cell_input_text("mnt_anticipation", 12, afficheMontant($mnt_anticipation), "", "", true));
    $table->add_cell(new TABLE_cell_input_text("mnt_anticipation_actuel", 12, afficheMontant($info_doss["interet_remb_anticipe"]), "", "", true));
    if ($info_doss["interet_remb_anticipe"] > 0){
      $SESSION_VARS['int_remb_anticipation_old'] = $info_doss["interet_remb_anticipe"];
    }

    $js = "function MiseAJourMntAnticipation(){
  mnt_solde =recupMontant(document.ADForm.mnt_solde.value);
  tx_anti = document.ADForm.tx_anticipation.value;
  mnt_anti = document.ADForm.mnt_anticipation;
  mnt_anti.value = formateMontant(Math.round(mnt_solde * (tx_anti/100)))  ;
  }";
    $formEcheancier->addJS(JSP_FORM, "JS1", $js);
  }
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    ...
    $HTML_code_echeancier = HTML_echeancier_remboursement_anticipe($parametre, $echeancier, $champs_modifiables, $Dossier, $Produit);
...
/*
    $size_echeance = sizeof($echeancier);print_rn($size_echeance);
    $date_jour = date("d");
    $date_mois = date("m");
    $date_annee = date("Y");
    ...
    foreach($echeancier as $key => $value){
      if(date($value['date_ech']) <= $date_fin_mois){
        $total_int_fin += $value['solde_int'];
        $id_array1 = $key + 1; //print_rn("date ech =>".date($value['date_ech'])." Date now =>".$date_debut_mois." et le Key =>".$key);
      }
      if(date($value['date_ech']) >= $date_fin_mois){
        //$total_int_fin += $value['solde_int'];
        $id_array2 = $key; //print_rn("date ech =>".date($value['date_ech'])." Date now =>".$date_debut_mois." et le Key =>".$key);
      }
    }print_rn($total_int_fin);
    $prems_id_ech = $echeancier[0]['id_ech'];
    $dern_id_ech = $echeancier[$size_echeance-1]['id_ech'];print_rn($dern_id_ech);
    $prems_id_ech += 1;
    //$id_array += 1;
    $js_raz = "
    var i=0;
    for (i= ".$id_array1."; i<= ".$id_array2.";i++){
      document.getElementsByName('solde_int".$id_doss."'+i).item(0).value = parseInt(0);

    }
    document.getElementsByName('total_int".$id_doss."').item(0).value =0;
    ";
*/

    echo   $SESSION_VARS['infos_doss'][$id_doss]["total_pen"];
    // Sauvegarder total_int, total_pen et total_gar si c'est le premier passage dans Tre-2
    ...
    $formEcheancier->addJS(JSP_FORM, "RAZ", $js_raz);
    ...
    if ($compteDossier==sizeof($SESSION_VARS['infos_doss'])) {
     ...
     // $formEcheancier->addFormButton(1,1,"retour",_("Précédent"),TYPB_SUBMIT);
      ...
      // Propriétés des boutons
      //$formEcheancier->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, "Tre-1");
      $formEcheancier->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Tre-3");
...
else if ($global_nom_ecran == 'Tre-3') {
    ...
    foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
      ...
      if ($compteDossier==sizeof($SESSION_VARS['infos_doss'])) {
       ...
       // $formEcheancier->addFormButton(1,1,"retour",_("Modifier"),TYPB_SUBMIT);
        ...
        // Propriétés des boutons
       // $formEcheancier->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, "Tre-2");
        $formEcheancier->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Tre-4");
        $formEcheancier->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Gen-11");
        //$formEcheancier->setFormButtonProperties("retour", BUTP_CHECK_FORM, false);
...
/*{{{ Tre-4 : Confirmation de l'enregistrement du nouvel échéancier */
else if ($global_nom_ecran == 'Tre-4') {
  global $colt_error;

  $echeancier_modifie =  $SESSION_VARS["echeancier_modifie"];
  $echeancier_original = $SESSION_VARS["echeancier_original"];

  $mntCapEncours = getSoldeCapital($SESSION_VARS['id_doss_anti']);
  $mnt_anti_calculer = $mntCapEncours * ($SESSION_VARS["tx_anticipation"]/100);
  ...
  $data_update = array(
    "interet_remb_anticipe" => round($mnt_anti_calculer)
  );
  $updateMntIntAnticipe = updateInteretAnticipe($SESSION_VARS['id_doss_anti'],$data_update);
  $Error = $updateMntIntAnticipe[0];
