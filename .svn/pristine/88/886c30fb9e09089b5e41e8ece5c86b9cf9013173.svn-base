  ...
  <xsl:template match="infos_synthetiques">
    ...
    <fo:list-block>
      <fo:list-item>
        ...
        <fo:list-item-body>
          <fo:block><xsl:value-of select="$point_liste" disable-output-escaping="yes"/>Nombre de credits en retard et sain: <xsl:value-of select="nbre_credits_retard"/> (<xsl:value-of select="prc_credits_retard"/>)</fo:block>
        </fo:list-item-body>
      </fo:list-item>
    </fo:list-block>
    <fo:list-block>
