...
/**
 * [609] Correction des dossier de crédit
 * Cette opération comprends les écrans :
 * - LCdd-1 : Sélection du type de correction
 * - LCdd-2 : sélection d'un dossier de crédit
 * - LCdd-3 : Affichage des informations dépendant du type de correction
 * - LCdd-4 : Affichage des remboursements effectués sur le crédit
 * - LCdd-5 : Affichage des remboursements à annuler
 * - LCdd-6 : Confirmation annulation des remboursements
 * - LCdd-7 : Affichage du dossier à supprimer
 * - LCdd-8 : confirmation suppression dossier de crédit
 * @package Credit
 */
...
/*{{{ LCdd-1 : Sélection de l'opération de correction d'un dossier de crédit */
if ($global_nom_ecran == "LCdd-1") {
  ...
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "LCdd-2");
...
/*{{{ LCdd-2 : Sélection d'un dossier de crédit */
else if ($global_nom_ecran == "LCdd-2") {

  // Si l'on vient de l'ecran LCdd-1, recuperer les valeurs de session
  if (strstr($global_nom_ecran_prec,"LCdd-1")) {
  ...
  if ($SESSION_VARS['infos_client']['statut_juridique'] == 4) {
    ...
    foreach($dossiers_fictifs as $id=>$value) {
      ...
      // Récupération des dossiers individuels réels (dans ad_dcr) suivant l'opération de correction
        if ($SESSION_VARS["type_cor"] == 1){
            if (($val['is_ligne_credit'] == 't') AND ($val['id_dcr_grp_sol'] == $id) AND ($val['etat'] == 5 or $val['etat'] == 6 or $val['etat'] == 9)) {
...
//  if ( $SESSION_VARS["type_cor"] == 1) { // Correction d'un remboursement
  ...
  */
  ...
  $Myform->setFieldProperties("source", FIELDP_HAS_CHOICE_AUCUN, false);
  ...
  // Propriétés des boutons
  $Myform->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "LCdd-1");
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "LCdd-3");
  $Myform->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);
  ...
  echo $Myform->getHTML();
      ...
      $Myform->addButton("BUTTON_OK", 'LCdd-1');
      $Myform->buildHTML();
      echo $Myform->getHTML();
      die();
  }
...
/*{{{ LCdd-3 : informations sur le dossier de credit */
else if ($global_nom_ecran == "LCdd-3") {
...
if (!isset($SESSION_VARS["id_doss"])) {
  ...
  if ($SESSION_VARS['dossiers'][$HTML_GEN_LSB_id_doss]['gs_cat'] != 2 ) { // dossier individuel
    ...
    foreach($dossiers_membre as $id_doss=>$val_doss) {
      ...
      $whereCl_dossier =" AND id_doss =".$SESSION_VARS["id_doss"];
	    ...
	    // Si on vient de LCdd-2, on récupère les infos de la BD
    if (strstr($global_nom_ecran_prec,"LCdd-2")) {
    	unset($SESSION_VARS['infos_doss']);
  ...
  if ( $SESSION_VARS["type_cor"] == 1) { // Correction d'un remboursement
    ...
    //S'il n'ya pas de remboursement pour ce dossier de crédit, on
        if(!$has_remb){
            ...
            $Myform->addButton("BUTTON_OK", 'LCdd-2');
            $Myform->buildHTML();
            echo $Myform->getHTML();
            die();
        }
    ...
    $MyPage->setFormButtonProperties("valid", BUTP_PROCHAIN_ECRAN, "LCdd-4");
    $MyPage->addFormButton(1,2,"retour",_("Précédent"),TYPB_SUBMIT);
    $MyPage->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, "LCdd-2");
  ...
  elseif( $SESSION_VARS["type_cor"] == 2) { // Suppression d'un dossier de crédit
    ...
    //Boutons
    $MyPage->addFormButton(1,1,"valid",_("Valider"),TYPB_SUBMIT);
    $MyPage->setFormButtonProperties("valid", BUTP_PROCHAIN_ECRAN, "LCdd-7");
    $MyPage->addFormButton(1,2,"retour",_("Précédent"),TYPB_SUBMIT);
    $MyPage->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, "LCdd-2");
    $MyPage->setFormButtonProperties("retour", BUTP_CHECK_FORM, false);
    $MyPage->addFormButton(1,3,"annuler",_("Annuler"),TYPB_SUBMIT);
...
/*{{{ LCdd-4 : Choix des remboursements à supprimer */
else if ($global_nom_ecran == "LCdd-4") {

	$formRemb = new HTML_GEN2(_("Les remboursements à corriger"));
	if (strstr($global_nom_ecran_prec,"LCdd-3")) {
    ...
    $MyformErr->addButton("BUTTON_OK", 'LCdd-3');
    $MyformErr->buildHTML();
...
//				$rembs = getRemboursement($whereCond);
//				// mettre les informations de remboursements dans la session pour l'ecran LCdd-5
//				$SESSION_VARS['infos_remb'][$id_doss][$id_ech] = $rembs;
  ...
  $formRemb->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN,"LCdd-5");

  $formRemb->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Lcr-1");
  $formRemb->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);
  $formRemb->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, "LCdd-3");
...
/*{{{ LCdd-5 : confirmation annulation */
else if ($global_nom_ecran == "LCdd-5") {
	...
	if (strstr($global_nom_ecran_prec,"LCdd-4")) {
    ...
    $MyformErr->addButton("BUTTON_OK", 'LCdd-4');
    $MyformErr->buildHTML();
    echo $MyformErr->getHTML();
    die();
	}
  ...
  $formRemb->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN,"LCdd-6");

  $formRemb->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Lcr-1");
  $formRemb->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);
  $formRemb->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, "LCdd-4");
  $formRemb->setFormButtonProperties("retour", BUTP_CHECK_FORM, false);
  $formRemb->buildHTML();
  //echo  $HTML_code;
  echo $formRemb->getHTML();
...
/*{{{ LCdd-6 : Annulation remboursement */
    ...
    $formRemb->addButton(BUTTON_OK, "Lcr-1");
    $formRemb->buildHTML();
...
/*{{{ LCdd-7 : Suppression d'un dossier de crédit */
else if ($global_nom_ecran == "LCdd-7") {
    $MyPage = new HTML_GEN2(_("Suppression dossier de crédit"));
    ...
    //Boutons
    $MyPage->addFormButton(1,1,"valid",_("Valider"),TYPB_SUBMIT);
    $MyPage->setFormButtonProperties("valid", BUTP_PROCHAIN_ECRAN, "LCdd-8");
    $MyPage->addFormButton(1,3,"annuler",_("Annuler"),TYPB_SUBMIT);
    $MyPage->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Lcr-1");
    $MyPage->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);
...
/*{{{ LCdd-8 : Suppression dossier de crédit */
else if ($global_nom_ecran == "LCdd-8") {
	$Myform = new HTML_GEN2(_("Suppression d'un dossier de crédit"));
	$source = $SESSION_VARS['source'];
	$myErr = supprimeDossier($source, $global_id_guichet, $SESSION_VARS['infos_doss'], 609);
  if ($myErr->errCode != NO_ERR) {
   $Myform = new HTML_erreur(_("Echec lors de la suppression du dossier "));
   $Myform->setMessage("Erreur : ".$myErr->param);
   $Myform->addButton(BUTTON_OK, "Lcr-1");
   $Myform->buildHTML();
