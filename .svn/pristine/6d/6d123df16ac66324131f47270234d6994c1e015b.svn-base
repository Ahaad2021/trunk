...
/**
 * [144] Approbation raccourcissement de la durée du crédit
 * Cette opération comprends les écrans :
 * - Ard-1 : sélection d'un dossier de crédit
 * - Ard-2 : approbation d'un dossier de crédit
 * - Ard-3 : affichage de l'échéancier
 * - Ard-4 : confirmation approbation d'un dossier de crédit
 * @package Credit
 */
...
if ($global_nom_ecran == "Ard-1") {
  ...
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Ard-2");
...
/*{{{ Ard-2 : Approbation d'un dossier de crédit */
else if ($global_nom_ecran == "Ard-2") {
  global $adsys;
  // Si on vient de Ard-1, on récupère les infos de la BD
  if (strstr($global_nom_ecran_prec,"Ard-1")) {
  ...
  $Myform = new HTML_GEN2(_("Approbation raccourcissement du dossier de crédit"));

  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    $nom_cli = getClientName($val_doss['id_client']);

    if($val_doss['etat'] == 15) {
        $Myform->addHTMLExtraCode("espace".$id_doss,"<br/><b><p align=\"center\"><b> ".sprintf(_("Approbation Raccourcissement du dossier N° %s de %s"),$id_doss,$nom_cli)."</b></p>");
    }
    ...
    if ($SESSION_VARS['infos_doss'][$id_doss]['etat'] == 15) { // Raccourcissement
      ...
      $his_obj = Historisation::getListDossierHis($id_doss, Historisation::MOD_TYPE_RACCOURCI, 'f');
    ...
    $js_check .="\t\t
			    ...
			    if(selected_echeances < 1 || selected_echeances > allowed_echeances) {
				    msg +='".$msgControlDuree."\\n';
				    document.ADForm.nbr_echeances_souhaite$id_doss.value =  parseInt($tmp_reech_duree);
				    ADFormValid=false;
			    }
			    ";
  ...
  // Propriétés des boutons
  $Myform->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Gen-11");
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Ard-3");
...
/*{{{ Ard-3 : Affichage de l'échéancier */
else if ($global_nom_ecran == "Ard-3") {
	$HTML_code = '';
	...
	foreach ( $SESSION_VARS ['infos_doss'] as $id_doss => $val_doss )
	{	// 15: En attente approbation raccourcissement durée	
		if ($val_doss ["last_etat"] == 15) 
		{		
			...
			$echeancier = calcul_echeancier_theorique_raccourci($id_prod, $SESSION_VARS['infos_doss'][$id_doss]["nouveau_cap"], $SESSION_VARS['infos_doss'][$id_doss]['nbr_echeances_souhaite'], $differe_jours, $differe_ech, NULL, 1, $id_doss);
...
/*{{{ Ard-4 : Confirmation approbation d'un dossier de crédit */
else if ($global_nom_ecran == "Ard-4") {
  // Préparation des dossiers à approuver
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    // Infos du dossier
    if($val_doss['last_etat'] == 15) { // dossier en attente de raccourcissement
      ...
      // En souffrance (donc moratoire)
        ...
        // Erreur
          ...
          // Il s'agit d'un rééchelonnement, le terme ne varie pas
      ...
      if ($val_doss["id_ech"] > 0 ) {
        ...
        $DATA[$id_doss]['infos_ech']['mnt_reech'] = '0'; //$val_doss["mnt_reech"];
  ...
  $myErr = approbationDcrCredit($DATA);
  if ($myErr->errCode == NO_ERR) {
    $msg = new HTML_message(_("Confirmation approbation raccourcissement du dossier de crédit"));
    $message = _("Le dossier de crédit N°".$id_doss." est passé avec succès à l'état accepté !");
