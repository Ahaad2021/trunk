...
if ($etape == 1) {
  ...
  // Création d'une fonction JS qui avertit l'utilisateur qui a saisi un taux supérieur au taux indicatif
...
/*{{{ Etape 2 */
  ...
  if (($SESSION_VARS["change"]["envoi_reste"] == 1) && ($tmp["alert"] == true)) {
    ...
    $Code_Js = "alert('"._("Cette opération de change n\\'est pas permise car les billets et les unités des deux devises rendent le calcul impossible.")." ".sprintf(_("Vous pouvez corriger ce prolème en changeant directement le montant de %s en %s"),afficheMontant($ctrval, $SESSION_VARS["change"]["devise_achetee"]),$SESSION_VARS["change"]["devise_vendue"])."');window.close();";
  } else {
