...
END;
' LANGUAGE 'plpgsql';
-- Fonction qui calcule la C/V de montant devise1 en devise2  de l'agence id_agence
CREATE OR REPLACE FUNCTION CalculeCV(NUMERIC(30,6), CHAR(3), CHAR(3),INTEGER) RETURNS NUMERIC(30,6) AS '
DECLARE
	montant ALIAS FOR $1;
	devise1 ALIAS FOR $2;
  devise2 ALIAS FOR $3;
...
BEGIN
		IF devise1 = devise2 THEN
	  	RETURN montant;
   	END IF;
		SELECT INTO tx_dev1 taux_indicatif FROM devise WHERE  code_devise = devise1 AND id_ag=id_agence;
		SELECT INTO tx_dev2 taux_indicatif FROM devise WHERE  code_devise = devise2 AND id_ag=id_agence;
		taux := tx_dev2 / tx_dev1;
		cv_montant := (montant * taux);
		SELECT INTO prec precision FROM devise WHERE  code_devise = devise2 AND id_ag=id_agence;
		cv_montant := ROUND(cv_montant, prec);
		RETURN cv_montant;
END;
' LANGUAGE 'plpgsql';
