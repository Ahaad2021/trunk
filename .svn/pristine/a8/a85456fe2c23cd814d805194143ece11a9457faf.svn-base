...
function make_gzip($fichier) {
  ...
  $retour = exec(escapeshellcmd("PGPASSWORD=$DB_pass psql -U $DB_user -d $DB_name ")."-c 'SHOW port;' > /dev/null", $output, $code_retour);

	if ($code_retour == 0){
      $retour = exec(escapeshellcmd("PGPASSWORD=$DB_pass pg_dump -U $DB_user $DB_name")." | gzip > $fichier", $output, $code_retour);
    }
	else{
...
function backup_db() {
  ...
  $fichier = "$lib_path/backup/batch/" . $date_annee . "-" . $date_mois . "-" . $date_jour . $suffix;
  if ($disable_backup)
  ...
  while ($file = readdir($dir)) {
    ...
    if (endsWith($file, $suffix) || (strlen($file) == 17 && substr($file, -7) == '.sql.gz') ) {
      $str_date_img = substr($file, 0, 10); //example: get only the part "2013-01-05" from "2013-01-05.sql.gz" or from "2013-01-05.meck_domoni_10.sql.gz"
