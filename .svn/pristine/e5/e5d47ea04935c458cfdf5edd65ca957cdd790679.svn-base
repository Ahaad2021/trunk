<?php
/*
 * Created on 27 févr. 08
 *
 * To change the template for this generated file go to
 * Window - Preferences - PHPeclipse - PHP - Code Templates
 */
 /**
  * Souscription de  part social par lot via un fichier
  *
  */
require_once 'lib/dbProcedures/guichet.php';

/*{{{ Psf-1 : Choix de la source des fonds */
if ($global_nom_ecran == "Psf-1") {

  $MyPage = new HTML_GEN2(_("Souscription de part social par lot via fichier"));

  $choices = array(1 => _("Cash"), 2 => _("Correspondant bancaire"), 3 => _("Compte de base"));
  ...
  $MyPage->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, 'Psf-2');
...
/*{{{ Psf-2 : Récupération du fichier de données */
else if ($global_nom_ecran == "Psf-2") {
  ...
  $htm1 .= "<p align=\"center\"> <INPUT type=\"submit\" value=\"Envoyer\" onclick=\"document.ADForm.m_agc.value='".$_REQUEST['m_agc']."';document.ADForm.prochain_ecran.value='Psf-2';\"/> </p>";
  $htm1 .= "<br />";
  ...
  $MyPage->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, 'Psf-3');
  $MyPage->setFormButtonProperties("precedent", BUTP_PROCHAIN_ECRAN, 'Psf-1');
...
/*{{{ Psf-3 : Demande de confirmation de la Souscription de part social par lot via fichier */
else if ($global_nom_ecran == "Psf-3") {

  $MyErr = parse_ps_fichier_lot($SESSION_VARS['fichier_lot']);
  ...
  if ($MyErr->errCode != NO_ERR) {
    ...
    $html_err->addButton("BUTTON_OK", 'Psf-2');
    $html_err->buildHTML();
    echo $html_err->HTML_code;
  } else if ($MyErr->errCode == NO_ERR) {
    $param = $MyErr->param;

    $MyPage = new HTML_GEN2(_("Demande de confirmation de la souscription des parts sociales par lot via fichier"));

    $MyPage->addField("montant", _("Montant"), TYPC_MNT);
    ...
    $MyPage->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, 'Psf-4');
    $MyPage->setFormButtonProperties("precedent", BUTP_PROCHAIN_ECRAN, 'Psf-2');
    $MyPage->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, 'Gen-6');
...
/*{{{ Psf-4 : Confirmation de la souscription de part social  par lot via fichier */
else if ($global_nom_ecran == "Psf-4") {

  $MyErr = traite_ps_fichier_lot($SESSION_VARS["fichier_lot"], $SESSION_VARS['SOURCE']);

  if ($MyErr->errCode == NO_ERR) {
    $html_mess = new HTML_message(_("Confirmation souscription parts sociales par lot via fichier"));
    $html_mess->setMessage(_("La souscription des parts sociales par lot via fichier s'est déroulée avec succès !"));
    $html_mess->addButton(BUTTON_OK, "Gen-6");
    $html_mess->buildHTML();
    echo $html_mess->HTML_code;
  } else {
    $param = $MyErr->param;
    $html_err = new HTML_erreur(_("Echec de la souscription des parts sociales pat lot via fichier"));
    $msg = _("Erreur : ").$error[$MyErr->errCode];
    if ($param != NULL) {
    	if(is_array($param)){
    		foreach($param as $key => $val){
