...
function verif_parametrage() {
  ...
  $donnees_ag = getAgenceDatas($id_agence); // $global_id_agence
...
function verif_conditions() {
  ...
  global $date_mois;
  global $date_annee;
  global $dbHandler, $DB_user, $DB_name, $DB_cluster, $batch_db_host, $DB_pass;
  global $global_id_agence;
  ...
  $retour = exec(escapeshellcmd("PGPASSWORD=$DB_pass psql -h $batch_db_host -U $DB_user -d $DB_name")." -c 'SHOW port;' > /dev/null", $output, $code_psql);
  if ($code_psql != 0)
    erreur("verif_conditions()", _("Le batch ne pourra pas faire une sauvegarde de la base, problème de droits d'accès. psql -U $DB_user $DB_name"));
  //Verif statut agence
...
function callback_overwrite_date_compta(&$value, $key)
{
  ...
  if (trim($date_total) != '') {
    if (in_array($key, $dates_arr)) {
      if ($is_type_opt == true) {
        $value = $date_total_type_opt;
        if ($key == "date_comptable") {
          $is_type_opt = false;
        }
      } else {
        $value = $date_total_local;
      }
    }
  }
...
/**
 * Fonction qui met a jour le champ num_cpte_comptable dans ad_cpt
