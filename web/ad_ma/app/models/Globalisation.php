...
class Globalisation extends BaseModel {
  ...
  public function getPeriode($date_total){
    ...
    $sql = "select (case when date_debut_solde > date('$date_total') then 1 else 2 end) as period, id_saison,id_annee, (case when date_debut_solde > date('$date_total') then date_debut else date_debut_solde end) as date_debut, (case when date_debut_solde > date('$date_total') then date_debut_solde else date_fin_solde end) as date_fin  from ec_saison_culturale where id_ag=".$this->getIdAgence();
    $result = $this->getDbConn()->prepareFetchAll($sql);
  ...
  public function getDateSaison($id_saison){
  ...
  public function deleteView(){
    ...
    $result = $this->getDbConn()->exec($sql);
    if($result===FALSE || count($result)<0) {
      $this->getDbConn()->rollBack(); // Roll back
      signalErreur(__FILE__, __LINE__, __FUNCTION__);
    }
...
INNER JOIN ec_commande_detail b ON a.id_commande = b.id_commande "; //b.date_creation, b.date_modif,
    ...
    if ($saison != null && $saison > 0 && $date_debut != null && $date_fin != null){
      ...
      if ($periode != null && $periode == 1){
        ...
        $sql.= " LEFT JOIN ec_paiement_commande p ON p.id_detail_commande = b.id_detail AND date(p.date_creation) >= date('$date_debut') AND date(p.date_creation) <= date('$date_fin')".  $where ."  AND a.id_saison <= $saison";
      }
      else {
        $sql.= " LEFT JOIN ec_paiement_commande p ON p.id_detail_commande = b.id_detail AND date(p.date_creation) >= date('$date_debut') AND date(p.date_creation) <= date('$date_fin')".  $where ." AND a.id_saison <= $saison"; //(b.date_creation >= '$date_debut' AND b.date_creation <= '$date_fin') AND
      }
    }
    else {
    ...
    $sql.= " ORDER BY a.id_benef ASC, b.id_produit ASC)  data"; //b.date_creation, b.date_modif, //  GROUP BY a.id_benef, b.id_produit, $groupBy
    if ($periode != null && $periode == 2){
    ...
    if ($periode == 1 ){
      ...
      $sql .= " GROUP BY data.id_benef, data.id_saison, data.id_remb, data.id_produit, data.prix_total, data.montant_solde";
    }
    $sql.=")";
    $result = $this->getDbConn()->exec($sql);
    if($result===FALSE || count($result)<0) {
