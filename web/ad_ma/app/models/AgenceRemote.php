...
/**
 * Description de la classe AgenceRemote
 *
 * @author danilo
 */
class AgenceRemote {
     ...
     * @return array Tableau associatif avec les agences trouvés.
     */
    public static function getListRemoteAgence($isOnline = FALSE) {
        global $dbHandler, $global_id_agence;

        $db = $dbHandler->openConnection();
        $sql = "SELECT * FROM adsys_multi_agence WHERE id_agc!=$global_id_agence AND is_agence_siege='f' ORDER BY id_agc ASC";
        //$sql = "SELECT * FROM adsys_multi_agence ORDER BY id_agc ASC";
        ...
        while ($prod = $result->fetchrow(DB_FETCHMODE_ASSOC))
        {
            ...
            if($isOnline){
                require_once 'DB.php';
                $agenceInfos = AgenceRemote::getRemoteAgenceInfo($prod["id_agc"], DB_FETCHMODE_ASSOC);
