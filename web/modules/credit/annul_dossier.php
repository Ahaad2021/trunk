...
/**
 * [120] Annulation d'un dossier de crédit
 * Cette opération comprends les écrans :
 * - And-1 : sélection d'un dossier de crédit
 * - And-2 : affichage du dossier de crédit
 * - And-3 : confirmation de l'annulation
 * @package Credit
 */
...
if ($global_nom_ecran == "And-1") {
  ...
  $whereCl = " AND is_ligne_credit='f' AND mode_calc_int!=5 AND ((etat=1) OR ((etat=7 AND periodicite <> 6)) OR (etat=2) OR (etat=14))"; // Le dossier doit être en attente de décision ou en attente de Rééch/Moratoire
  ...
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "And-2");
...
else if ($global_nom_ecran == "And-2") {
  // Si on vient de Apd-1, on récupère les infos de la BD
  if (strstr($global_nom_ecran_prec,"And-1")) {
...
// Test de la date d'annulation
  ...
  // Propriétés des boutons
  $Myform->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Gen-11");
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "And-3");
  $Myform->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);
...
/*{{{ And-3 : Confirmation de l'annulation */
else if ($global_nom_ecran == "And-3") {
  $annulation=0;
  $moratoire=0;
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    if (in_array($val_doss["etat"], array(1, 2))) {
    ...
    } else if (in_array($val_doss["etat"], array(7, 14))) {
      //Remplissage de $DATA avec les données postées au serveur.
      if($val_doss["etat"]==7) {
        $DATA[$id_doss]["nouv_duree_mois"] = 0;// durée
      }elseif($val_doss["etat"]==14) {
      ...
      $html_err = new HTML_erreur(_("Echec lors du rééchelonnement/moratoire.")." ");
      $html_err->setMessage(_("Le rééchelonnement/moratoire n'a pas été annulé")." : ".$error[$myErr->errCode].$myErr->param);
  ...
  if ($annulation!=0) {
    ...
    if (annulerCredit($DATA,$DATA_GAR)) {
    ...
    if ($myErr->errCode == NO_ERR) {
      ...
      if(is_array($modif_id_doss_arr) && count($modif_id_doss_arr) > 0) {
        foreach($modif_id_doss_arr as $key=>$id_doss) {
          ...
          $HisObj->deleteDossierHis($id_doss, 1, 'f');

          unset($HisObj);
        }
      }

      $msg = new HTML_message(_("Confirmation annulation de Rééchelonnement/Moratoire de crédit"));
      if ($moratoire > 1)
        $msg->setMessage(_("Le rééchelonnement/moratoire a été annulé pour ces dossier de crédit !"));
      else
        $msg->setMessage(_("Le rééchelonnement/moratoire a été annulé pour ce dossier de crédit !"));
      $msg->addButton(BUTTON_OK,"Gen-11");
      $msg->buildHTML();
      echo $msg->HTML_code;
    }
