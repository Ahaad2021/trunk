...
/**
 * [602] Rejet dossier de crédit
 *
 * Cette opération comprends les écrans :
 * - LRfd-1 : sélection d'un dossier de crédit
 * - LRfd-2 : rejet d'un dossier de crédit
 * - LRfd-3 : confirmation du rejet
 * @package Credit
 */
...
if ($global_nom_ecran == "LRfd-1") {
  ...
  $whereCl = " AND is_ligne_credit='t' AND mode_calc_int=5 AND etat=1"; // Le dossier droit être en attente de décision
  ...
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "LRfd-2");
...
/*{{{ LRfd-2 : Rejet d'un dossier de crédit */
...
else if ($global_nom_ecran == "LRfd-2") {
  // Si on vient de Apd-1, on récupère les infos de la BD
  if (strstr($global_nom_ecran_prec,"LRfd-1")) {
  ...
  // Propriétés des boutons
  $Myform->setFormButtonProperties("annuler", BUTP_PROCHAIN_ECRAN, "Lcr-1");
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "LRfd-3");
  $Myform->setFormButtonProperties("annuler", BUTP_CHECK_FORM, false);
...
/*{{{ LRfd-3 : Confirmation du rejet */
else if ($global_nom_ecran == "LRfd-3") {
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    if ($val_doss["etat"]==1) {
  ...
  // Rejet du dossier
  $myErr = rejetCredit($DATA, $DATA_GAR,602);
