...
/**
 * [141] Demande modification de la date de remboursement
 * 
 * Cette opération comprends les écrans :
 * - Mdr-1 : sélection d'un dossier de crédit
 * - Mdr-2 : échéancier actuel à modifier
 * - Mdr-3 : nouvel échéancier
 * - Mdr-4 : confirmation enregistrement
 * 
 * @package Credit
 */
...
if ($global_nom_ecran == "Mdr-1") {
  ...
  // Récupération des dossiers individuels réels (dans ad_dcr) ayant l'état dossier "Fonds déboursés" et état crédit "SAIN"
  $whereCl = " AND (etat=5 AND cre_etat=1) ";
  ...
  if (is_array($dossiers_reels))
    ...
    if ($value['gs_cat'] != 2) { // les dossiers pris en groupe doivent être déboursés via le groupe
      //if (allowed_demande_reech($id_doss)) {
  ...
  if ($SESSION_VARS['infos_client']['statut_juridique'] == 4) {
    ...
    foreach($dossiers_fictifs as $id=>$value) {
      ...
      if (($val['id_dcr_grp_sol'] == $id) AND ($val['etat'] == 5) AND ($val['cre_etat'] == 1)) {
        //if (allowed_demande_reech($id_doss)) {
  ...
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Mdr-2");
...
/*{{{ Mdr-2 : échéancier courant à modifier */
elseif ($global_nom_ecran == 'Mdr-2') {
  global $adsys, $global_id_client;
  ...
  // Création du formulaire
  $Myform = new HTML_GEN2(_("Modification de la date de remboursement")); // Modification de la date de remboursement
  $js_duree = '';
  $HTML_code = '';
  $checkDate = '';
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    ...
    $Myform->setFieldProperties("ech_date_dem".$id_doss, FIELDP_DEFAULT, date("d/m/Y")); // Afficher la date du jour
  ...
  // Propriétés des boutons
  $Myform->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Mdr-3");
...
elseif ($global_nom_ecran == "Mdr-3") {
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    ...
    $ech_date_dem1 = php2pg($_POST["ech_date_dem$id_doss"]); // Récupéré la date de remboursement souhaité//date dem remb
    $ech_date_dem =  pg2phpDate($ech_date_dem1);
    ...
    $SESSION_VARS["moddateremb$id_doss"]['ech_date_dem'] = $ech_date_dem1; // Store date de remboursement souhaité in session
    ...
    //bon verification de la grandeur entre de date different
  ...
  $formEcheancier->setFormButtonProperties("valider", BUTP_PROCHAIN_ECRAN, "Mdr-4");
...
elseif ($global_nom_ecran == "Mdr-4") {
  ...
  foreach($SESSION_VARS['infos_doss'] as $id_doss=>$val_doss) {
    ...
    $DATA["ech_date_dem"] = $SESSION_VARS["moddateremb$id_doss"]['ech_date_dem'];
    ...
    if (is_array($DATA) && count($DATA)>0) {
        ...
        if(updateCredit($DATA["id_doss"], $UPDATE_DATA)) {
            ...
            if ($id_his == NULL) {
                // Log action
                $myErr = ajout_historique(141, $DATA["id_client"], "Demande modification de la date de remboursement (".$DATA["id_doss"].")", $global_nom_login, date("r"), NULL);
            ...
            $erreur = $HisObj->insertDossierHis($DATA["id_doss"], 1, $DATA["echeance_index"], $DATA["ech_date_dem"]);
  ...
  if ($has_error == FALSE) {
      $msg = new HTML_message(_("Confirmation demande modification de la date de remboursement"));
      ...
      if (is_array($id_doss_arr) && count($id_doss_arr) > 0) {
          ...
          $msg->setMessage(_("Les Dossiers de crédit ".$list_id_doss." ont été mis en attente de modification!"));
      } else {
        $msg->setMessage(_("Le Dossier de crédit N°".$id_doss_arr[0]." a été mis en attente de modification!"));
      }
      $msg->addButton(BUTTON_OK, "Gen-11");
      $msg->buildHTML();
      echo $msg->HTML_code;
  } else {
      $html_err = new HTML_erreur(_("Echec demande de modification de la date de remboursement."));
