...
/**
 * Recherche d'un client  et leur numero compte part sociale dans la BD exception compte source de transferts
    ...
    if ($existPM) {
      ...
      while (is_array($DATAS[$i])) {
        if (isset($DATAS[$i]["pm_raison_sociale"]))
          $xtHTML .= "\n<tr bgcolor=\"$colb_tableau\"><td><a OnClick=\"validateSearch(".addslashes($DATAS[$i]["id_client"]).")\" href=\"rech_client_ps.php?m_agc=".$_REQUEST['m_agc']."&choixCompte=".$SESSION_VARS['choixCompte']."&num_client_dest=".$num_client_dest."&client=".$DATAS[$i]["id_client"]."\">".$DATAS[$i]["id_client"]."</a></td><td>".$DATAS[$i]["pm_raison_sociale"]."</td></tr>";
        $i++;
      }
      $xtHTML .= "</table>";
    }
...
/*{{{ Cpt : Comptes du client trouvé */
  ...
  //getnum cpte parts sociale 361
    $ListeComptes = get_comptes_epargne_parts_sociale($client, $devise);
  ...
  if (!isset($SESSION_VARS['devise_cpte_dest'])) {
  	 $JScode1 =  "
              function validateSearch(id, num)
            {
              ...
              window.opener.document.ADForm.".$SESSION_VARS['cpt_dest']."_hdd.value = id;
              window.close();
            }
              ";
  } else {
...
else signalErreur(__FILE__,__LINE__,__FUNCTION__, _("[rech_client_ps.php] Problème de propagation des variables"));
