 ...
 * - Vcp-1 : Choix d'un compte et du type de consultation (par dates ou par n derniers mouvements)
 * - Vcp-2 : Affichage de la liste des mouvements
 * - Vcp-3 et Vcp-4 : Impression PDF et export CSV de la liste des mouvements
 * @package Epargne
 */
...
if ($global_nom_ecran == "Vcp-1") {
  //afficher la liste des comptes du client puis dans l'écran suivant l'historique des derniers mouvements
  
  //trace dans ad_his
  ajout_historique (80, $global_id_client, 'consultation d\'un compte', $global_nom_login, date("r"), NULL);
  ...
  $html->setFormButtonProperties("ok", BUTP_PROCHAIN_ECRAN, 'Vcp-2');
...
/*{{{ Vcp-2 : Affichage des informations */
else if ($global_nom_ecran == "Vcp-2") {
  ...
  $html->setFormButtonProperties("retour", BUTP_PROCHAIN_ECRAN, 'Vcp-1');
  ...
  $html->setFormButtonProperties("pdf", BUTP_PROCHAIN_ECRAN, "Vcp-3");
  $html->addFormButton(1, 4, "csv", _("Export CSV"), TYPB_SUBMIT);
  $html->setFormButtonProperties("csv", BUTP_PROCHAIN_ECRAN, "Vcp-4");

  $html->buildHTML();
  echo $html->getHTML();
} /*}}}*/

/*{{{ Vcp-3 et Vcp-4 : Liste mouvements : impression pdf ou export CSV */
else if ($global_nom_ecran == "Vcp-3" || $global_nom_ecran == "Vcp-4") {
  ...
  if ($global_nom_ecran == "Vcp-4") {
    ...
    echo getShowCSVHTML("Gen-10", $csv_file);
  } else if ($global_nom_ecran == "Vcp-3") {
    ...
    //Message de confirmation + affichage du rapport dans une nouvelle fenêtre
    echo get_show_pdf_html("Gen-10", $fichier_pdf);
  }
}
/*}}}*/
