...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="1.0">
  ...
  <xsl:template match="schema_compta">
    <xsl:call-template name="titre_niv1">
      <xsl:with-param name="titre" select="'Liste des schémas comptables'"/>
    </xsl:call-template>
    <fo:table border="none" border-collapse="separate" border-separation.inline-progression-direction="10pt">
      ...
      <fo:table-header>
        <fo:table-row font-weight="bold">
          ...
          <fo:table-cell border-bottom-width="0.5pt" border-bottom-style="solid" border-bottom-color="black">
            <fo:block>Operation designation</fo:block>
          </fo:table-cell>
          <fo:table-cell border-bottom-width="0.5pt" border-bottom-style="solid" border-bottom-color="black">
            <fo:block>Debit to account</fo:block>
          </fo:table-cell>
          <fo:table-cell border-bottom-width="0.5pt" border-bottom-style="solid" border-bottom-color="black">
            <fo:block>Credit to account</fo:block>
          </fo:table-cell>
        </fo:table-row>
      </fo:table-header>
      <fo:table-body>
  ...
  <xsl:template match="detail_schema">
    <fo:table-row>
      ...
      <fo:table-cell>
        <fo:block space-before.optimum="0.2cm">
          <xsl:value-of select="cpte_debit"/>
        </fo:block>
      </fo:table-cell>
      <fo:table-cell>
        <fo:block space-before.optimum="0.2cm">
          <xsl:value-of select="cpte_credit"/>
        </fo:block>
      </fo:table-cell>
    </fo:table-row>
  </xsl:template>
