			...
			<fo:table border="none" border-collapse="separate" border-separation.inline-progression-direction="10pt">
				...
				<fo:table-header>
					<fo:table-row font-weight="bold">
						<fo:table-cell>
							<fo:block>N° compte</fo:block>
						</fo:table-cell>
						<fo:table-cell>
							<fo:block>Libellé comppte</fo:block>
						</fo:table-cell>
						<fo:table-cell>
							<fo:block text-align="center">Solde débiteur</fo:block>
						</fo:table-cell>
						<fo:table-cell>
							<fo:block text-align="center">Solde créditeur</fo:block>
						</fo:table-cell>
					</fo:table-row>
				</fo:table-header>
...
<xsl:template match="header_contextuel">
	...
	<fo:list-block>
		<fo:list-item>
			...
			<fo:list-item-body><fo:block><xsl:value-of select="$point_liste" disable-output-escaping="yes"/>Clôture : <xsl:value-of select="id_cloture"/></fo:block></fo:list-item-body>
		</fo:list-item>
	</fo:list-block>
	<fo:list-block>
		<fo:list-item>
			<fo:list-item-label><fo:block></fo:block></fo:list-item-label>
			<fo:list-item-body><fo:block><xsl:value-of select="$point_liste" disable-output-escaping="yes"/>Date clôture : <xsl:value-of select="date_cloture"/></fo:block></fo:list-item-body>
		</fo:list-item>
	</fo:list-block>
	<fo:list-block>
		<fo:list-item>
			<fo:list-item-label><fo:block></fo:block></fo:list-item-label>
			<fo:list-item-body><fo:block><xsl:value-of select="$point_liste" disable-output-escaping="yes"/>Exercice: <xsl:value-of select="id_exo"/></fo:block></fo:list-item-body>
		</fo:list-item>
	</fo:list-block>
</xsl:template>
...
<xsl:template match="compte">
	...
	<fo:table-row font-weight="bold">
		...
		<fo:table-cell border-top-width="0.5pt" border-top-style="solid" border-top-color="black">
			<fo:block text-align="right" space-before.optimum="0.2cm"><xsl:value-of select="solde_debit"/></fo:block>
		</fo:table-cell>
		<fo:table-cell border-top-width="0.5pt" border-top-style="solid" border-top-color="black">
			<fo:block text-align="right" space-before.optimum="0.2cm"><xsl:value-of select="solde_credit"/></fo:block>
		</fo:table-cell>
	</fo:table-row>
	</xsl:if>
	<xsl:if test="@total = '0'">
