...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="1.0">
  ...
  <xsl:template match="credit">
    <xsl:call-template name="titre_niv1">
      <xsl:with-param name="titre" select="'Crédit'"/>
    </xsl:call-template>
    <fo:table border-collapse="collapse" border-separation.inline-progression-direction="10pt" width="100%" table-layout="fixed">
      <xsl:call-template name="debut_table"/>
      <fo:table-body>
        <xsl:apply-templates select="cap_attendu/previsions">
          <xsl:with-param name="titre" select="'Capital attendu'"/>
        </xsl:apply-templates>
        <xsl:apply-templates select="int_attendu/previsions">
          <xsl:with-param name="titre" select="'Intérêts attendus'"/>
        </xsl:apply-templates>
      </fo:table-body>
    </fo:table>
  </xsl:template>
  <xsl:template match="epargne">
    <xsl:call-template name="titre_niv1">
      <xsl:with-param name="titre" select="'Epargne'"/>
    </xsl:call-template>
    <fo:table border-collapse="collapse" border-separation.inline-progression-direction="10pt" width="100%" table-layout="fixed">
      <xsl:call-template name="debut_table"/>
      <fo:table-body>
        <xsl:apply-templates select="ep_nantie/previsions">
          <xsl:with-param name="titre" select="'Epargne nantie'"/>
        </xsl:apply-templates>
