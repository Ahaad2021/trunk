...
DROP FUNCTION IF EXISTS f_getmouvementforproducer(text, numeric, text, integer);
---------------------------------------------------------------------------------------------------

----------- CREATE/ REPLACE fonction f_getmouvementforproducer(text, numeric, text, integer, text) -----------------
...
---------------------------------- DEBUT : Ticket AT-134 -------------------------------------------
CREATE OR REPLACE FUNCTION mise_a_jour_adsys_param_abonnement() RETURNS INT AS
$$
DECLARE
...
BEGIN
   ...
   IF EXISTS(SELECT * FROM information_schema.tables WHERE table_name = 'adsys_param_abonnement') THEN
       ALTER TABLE adsys_param_abonnement ALTER COLUMN valeur DROP NOT NULL;
   END IF;

   -- Insertion dans adsys_param_abonnement
   IF NOT EXISTS (SELECT * FROM adsys_param_abonnement WHERE cle = 'PREFIX_TELEPHONE') THEN
      INSERT INTO adsys_param_abonnement(cle, id_ag, libelle, lib_texte1) VALUES ('PREFIX_TELEPHONE', numagc(), 'Prefix numéro de téléphone authorisé', 'Numéro SMS doit commencer par les chiffres indiquer dans le champ valeur');
   END IF;
...
SELECT mise_a_jour_adsys_param_abonnement();
DROP FUNCTION mise_a_jour_adsys_param_abonnement();
------------------------------------ FIN : Ticket AT-134 ------------------------------------------