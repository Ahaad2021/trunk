...
ALGO :
     1/ Déterminer tous les comptes appartenant à  des produits d'épargne pour lesquels les intérets doivent etre prélevés aujourd'hui. Ce produits sont ceux pour lesquels 
         - le découvert max autorisé est > 0
         - le taux d'intéret débiteur est > 0
...
DECLARE 
  ...
  ListeComptes refcursor;               -- Liste des comptes à traiter
...
*/
  ...
  SELECT INTO annee extract(year from date_courante); -- AnnÃ©e actuelle
  ...
  CREATE TEMP TABLE cptes_a_prelever AS 
      SELECT a.id_cpte,a.solde as current_solde,a.devise,c.precision,b.tx_interet_debiteur, a.etat_cpte, b.cpte_cpta_prod_ep   
      FROM ad_cpt a, adsys_produit_epargne b, devise c 
      WHERE a.id_prod = b.id AND b.devise = c.code_devise AND a.etat_cpte = 1 
      AND b.nbre_jours_report_debit = jour AND b.tx_interet_debiteur > 0 order by id_cpte;


  -- est-ce qu''il y a quelque chose ?
  IF ((SELECT count(*) FROM cptes_a_prelever) > 0) THEN

    RAISE NOTICE ''Il y a des comptes sur lesquls on peut éventuellement prélever'';

  -- création des tables de travail

  CREATE TEMP TABLE listeDesDebits
    (
	id_cpte int4,
	devise char(3),
    ...
    FOR jour_traite IN REVERSE dern_jour_mois_traite .. 1 LOOP
      ...
      -- d''abord récupérer les mouvements au débit et au crédit pour tous les comptes pouvant être débités
	...
	SELECT a.id_cpte as cpte_cli_debit,date(date_traite), b.somme_mvt as somme_debit 
	...
	SELECT  a.id_cpte as cpte_cli_credit,date(date_traite), somme_mvt as somme_credit 
    ...
    END LOOP; 
      ...
      INSERT INTO var_soldes_total
        SELECT a.*,b.* FROM var_soldes_debit a FULL OUTER JOIN var_soldes_credit b 
        ON a.cpte_cli_debit = b.cpte_cli_credit AND a.date_traite_debit = b.date_traite_credit ;

      -- calculer le solde en date de valeur date_traite
      ...
      INSERT INTO cpte_a_prelever_soldes 
        SELECT a.cpte_cli_debit, a.date_traite_debit, b.devise,b.precision,b.cpte_cpta_prod_ep,b.tx_interet_debiteur,(b.current_solde - a.var_solde),
        abs(round(((b.current_solde - a.var_solde) *  b.tx_interet_debiteur / 30.0)::numeric(30,6), b.precision))  
        FROM var_soldes_total a, cptes_a_prelever b 
        WHERE a.cpte_cli_debit=b.id_cpte;
      ...
      DELETE FROM cpte_a_prelever_soldes where solde_valeur >= 0;
       ...
       -- Regrouper les intérets à payer par compte pour passer une seule écriture      
      INSERT INTO listeDesDebits
	SELECT id_cpte,devise,cpte_cpta_prod_ep,round(sum(int_deb)::numeric(30,6), precision)  
	FROM cpte_a_prelever_soldes 
	GROUP BY id_cpte,devise,cpte_cpta_prod_ep,precision 
	ORDER BY id_cpte;
      
	OPEN ListeComptes 
		FOR SELECT * FROM listeDesDebits;

	FETCH ListeComptes INTO ligne;
 	  ...
 	  WHILE FOUND LOOP
		...
		num_cpte_debit := ligne.cpte_cpta_prod_ep;
	     END IF;
	      ...
	      VALUES ((SELECT currval(''ad_his_id_his_seq'')),date(now()), ''Prélèvement intérets débiteurs'', 1, id_exo_courant,makeNumEcriture(1, id_exo_courant));
	...
	((SELECT currval(''ad_ecriture_seq'')),num_cpte_debit,ligne.id_cpte,''d'',ligne.interets_deb_a_payer,date(now()),ligne.devise);
	    ...
	    IF ligne.devise = code_dev_ref THEN       -- Pas de change à effectuer                    
            ...
            ELSE        -- Il faut mouvementer la position de change
