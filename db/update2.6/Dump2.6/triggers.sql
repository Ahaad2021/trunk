...
CREATE OR REPLACE FUNCTION trig_preleve_frais_attente() RETURNS trigger AS '
  ...
  BEGIN
   ...
   -- FAIRE LES TRAITEMENTS QUE SI LE SOLDE A ETE MODIFIE
   IF OLD.solde != NEW.solde THEN
    ...
    END IF ; -- Fin IF OLD.solde != NEW.solde THEN
...
CREATE OR REPLACE FUNCTION trig_calcul_solde_temps_reel() RETURNS trigger AS '
  BEGIN
    IF NEW.mode_calcul_int_cpte = 3 AND NEW.solde < NEW.solde_calcul_interets THEN
      ...
      UPDATE ad_cpt SET solde_calcul_interets = NEW.solde, date_solde_calcul_interets = date(now()) WHERE id_cpte = NEW.id_cpte;
    END IF;
    RETURN NULL;
  END;
' LANGUAGE 'plpgsql';
...
/*
 * Fonction appellée après chaque modification du champ solde de la table ad_cpt
...
CREATE OR REPLACE FUNCTION trig_utilisation_decouvert() RETURNS trigger AS '
  ...
  BEGIN
    ...
    SELECT INTO old_solde_dispo (OLD.solde
      - (SELECT mnt_bloq FROM ad_cpt WHERE id_cpte = OLD.id_cpte)
      - (SELECT mnt_min FROM adsys_produit_epargne WHERE id = OLD.id_prod));
    SELECT INTO new_solde_dispo (NEW.solde
      - (SELECT mnt_bloq FROM ad_cpt WHERE id_cpte = NEW.id_cpte)
      - (SELECT mnt_min FROM adsys_produit_epargne WHERE id = NEW.id_prod));

    IF new_solde_dispo < 0 AND old_solde_dispo >= 0 THEN
      -- Le découvert commence à être utilisé
      UPDATE ad_cpt SET decouvert_date_util = date(now()) WHERE id_cpte = NEW.id_cpte;

    ELSIF old_solde_dispo < 0 AND new_solde_dispo > old_solde_dispo THEN
      SELECT INTO annulation_auto decouvert_annul_auto FROM adsys_produit_epargne WHERE id = NEW.id_prod;
      IF annulation_auto THEN
        ...
        END IF;
        UPDATE ad_cpt SET decouvert_date_util = date_util, decouvert_max = new_decouvert_max WHERE id_cpte = NEW.id_cpte;
      END IF;
    END IF;
