...
DECLARE
 ...
 opening_list CURSOR for select R.type_rapport, R.id_prod, R.obj_dem, R.id_doss, date(R.cre_date_debloc) as cre_date_debloc, R.cre_id_cpte, R.cre_mnt_octr, R.etat_dossier, R.date_etat_doss,
			...
			(
				...
				dcr.is_ligne_credit
				from getportfeuilleview(date(p_date_debut)-1,numagc()) v
...
BEGIN
  ...
  WHILE FOUND LOOP
	...
	IF ligne.is_ligne_credit = 't' THEN
		...
		END LOOP;
		--capital debut periode
		SELECT INTO capital_debut_lcr SUM(CASE WHEN type_evnt = 2 THEN valeur WHEN type_evnt = 3 AND nature_evnt = 1 THEN -1*valeur ELSE 0 END) FROM ad_lcr_his
		...
		IF DATE(date_nettoyage) > DATE(p_date_debut) THEN
			...
			SELECT INTO cre_mnt_deb_per SUM(CASE WHEN type_evnt = 2 THEN valeur WHEN type_evnt = 3 AND nature_evnt = 1 THEN -1*valeur ELSE 0 END) FROM ad_lcr_his
			...
			WHERE id_doss = ligne.id_doss AND type_evnt = 3 AND nature_evnt = 1
	...
	IF ligne.type_rapport = '4-RADIE' OR (ligne.type_rapport = '2-SOLDE' AND ligne.perte_capital > 0) THEN
		...
		AND m.cpte_interne_cli = ligne.cre_id_cpte AND date(e.date_comptable) >= date(p_date_debut) AND date(e.date_comptable) <= date(p_date_fin)
	...
	SELECT INTO tot_mnt_remb_int sum(case when emh.type_operation IN (10,20,272,374) and emh.sens = 'c' then coalesce(emh.montant,0) --10,20,374
	when emh.type_operation IN (11,21,271) and emh.sens = 'd' then coalesce(emh.montant,0)*-1 else 0 end) --11,21,375
	...
	AND coalesce(emh.info_ecriture,emh.infos) = to_char(ligne.id_doss,'FM999999999MI') AND emh.type_operation IN (10,11,20,21,271,272,374) --10,11,20,21,374,375
	...
	SELECT INTO tot_mnt_remb_pen sum(case when emh.type_operation in (20,30) and emh.sens = 'c' then coalesce(emh.montant,0) --30,374
	when emh.type_operation in (21,31) and emh.sens = 'd' then coalesce(emh.montant,0)*-1 else 0 end) --31,375
	FROM inventaireCredit_ecr_mvt_his emh
	...
	AND coalesce(emh.info_ecriture,emh.infos) = to_char(ligne.id_doss,'FM999999999MI') AND emh.type_operation IN (20,21,30,31) --30,31,374,375
	AND date(emh.date_comptable) >= date(p_date_debut) AND date(emh.date_comptable) <= date(v_date_fin) GROUP BY coalesce(emh.info_ecriture,emh.infos); --GROUP BY e.info_ecriture; --interet de retard
...
BEGIN
  ...
  WHERE e.type_operation IN (10,11,20,21,30,31,271,272,374) AND date(e.date_comptable) >= date(p_date_debut) AND date(e.date_comptable) <= date(p_date_fin)
  ...
  IF p_etat_selecte = 3 OR p_etat_selecte = 4 THEN
	...
	FROM ad_sre WHERE id_doss = d.id_doss AND date(date_remb) >= date(p_date_debut) AND date(date_remb) <= date(p_date_fin)) AS tot_mnt_remb_cap,
	COALESCE((SELECT sum(CASE WHEN emh.type_operation IN (10,20,272,374) AND emh.sens = 'c' THEN COALESCE(emh.montant,0) WHEN emh.type_operation IN (11,21,271) AND emh.sens = 'd'
	...
	COALESCE(emh.info_ecriture,emh.infos) = to_char(d.id_doss,'FM999999999MI') AND emh.type_operation IN (10,11,20,21,271,272,374)
	--AND e.date_comptable >= date(p_date_debut) AND e.date_comptable <= date(p_date_fin)
	GROUP BY COALESCE(emh.info_ecriture,emh.infos)),0.000000) AS tot_mnt_remb_int,COALESCE((SELECT sum(CASE WHEN emh.type_operation IN (20,30) AND emh.sens = 'c'
	THEN COALESCE(emh.montant,0) WHEN emh.type_operation IN (21,31) AND emh.sens = 'd' THEN COALESCE(emh.montant,0)*-1 ELSE 0 END)
	FROM inventaireCredit_ecr_mvt_his emh WHERE emh.compte IN (SELECT cpte_cpta_prod_cr_pen FROM adsys_produit_credit
	WHERE id = d.id_prod) AND COALESCE(emh.info_ecriture,emh.infos) = to_char(d.id_doss,'FM999999999MI') AND emh.type_operation IN (20,21,30,31)
	--AND e.date_comptable >= date(p_date_debut) AND e.date_comptable <= date(p_date_fin)
	GROUP BY COALESCE(emh.info_ecriture,emh.infos)),0.000000) AS mnt_remb_pen, d.perte_capital FROM ad_dcr d INNER JOIN ad_cli c ON d.id_client = c.id_client
	INNER JOIN (SELECT DISTINCT dcr.id_doss FROM ad_dcr dcr WHERE dcr.etat = 6 AND date(dcr.date_etat) BETWEEN date(p_date_debut) AND date(p_date_fin)
	AND date(dcr.cre_date_debloc) >= date(p_date_debut) EXCEPT (SELECT DISTINCT id_doss FROM inventaireCredit)) d1 ON d.id_doss = d1.id_doss) s;
  END IF;
